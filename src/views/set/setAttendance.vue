      ...
      <div class="com-page-con">
        <div class="history-search">
          <span class="search-item">所属公司：
                <el-select v-model="search.orgIndexCode" clearable placeholder="请选择" @change="searchHandle">
                  <el-option
          ...
          <span class="search-item">所属驻地：
                <el-select v-model="search.regionIndexCode" clearable placeholder="请选择" @change="searchHandle">
                  <el-option
          ...
          <span class="search-item">
               <el-button v-if="roleId == '93'" type="warning" icon="el-icon-mouse" @click="staffAttendanceaPassAll()">一键满勤</el-button>
              </span>
        </div>
        <div class="table">
          <el-table
...
<script>
  ...
  export default {
    ...
    methods: {
      getOptions () {
        API.attendance.organization(null).then(da => {
          this.organizationOptions = da.data
        })
      ...
      staffAttendanceaPass (item, row) {
        ...
        this.$confirm('确定设为已签到, 是否继续?', '提示', {
          ...
          API.attendance.singlePass({
            ...
            attendanceDate: `${row.year}-${row.month < 10 ? '0' + row.month : row.month}-${item < 10 ? '0' + item : item}`,
          }).then(da => {
            if (da.status) {
              this.$message({
                type: 'success',
